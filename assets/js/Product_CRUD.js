// creating an array
let array = [];
function createJson(arr, id,LS_array) {
...
let create_card = JSON.parse(localStorage.getItem(LS_array));


// new array
for(let i = 0; i < create_card.length; i++){
  arr.push(create_card[i]);
...
for (let i = 0; i < arr.length; i++) {
  ...
  update_a.setAttribute("href","../pages/upload_page.html?id="+ arr[i]["id"])
  update_a.setAttribute("id" , "update")
  update_a.innerText = "Edit"
  crud_div.append(update_a)


  document.querySelector(id).append(card_div);
}
...
createJson(array, ".whole-div.container-fluid","card")